  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-400 ${navBg}`}>
          ...
          {user ? (
            <div className="relative group">
              <button className={`flex items-center gap-1.5 transition-colors duration-300 ${iconColor}`}>
                ...
                <span className={`hidden md:inline text-[0.75rem] font-medium ${
                  isHome && !scrolled ? 'text-white/85' : 'text-luvera-text'
                }`}>
                  {user.name.split(' ')[0]}
                </span>
              </button>
      ...
      <div className={`md:hidden bg-luvera-dark2 border-b border-white/10 shadow-lg transition-all duration-300 overflow-hidden ${
        mobileMenuOpen ? 'max-h-[400px] opacity-100 py-8' : 'max-h-0 opacity-0 py-0'
      }`}>
